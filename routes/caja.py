...
def caja():
    ...
    render_result = render_template('caja.html', productos=productos, clientes=clientes, total_carrito=total_carrito, carrito=carrito)

    return render_result

@caja_bp.route('/clienteht', methods=['POST'])
@proteger_ruta
def clienteht():
...
@caja_bp.route('/mostrarsuma', methods=['POST'])
@proteger_ruta
def suma():
    cliente_id = session.get('cliente_id')
    cur = mydb.cursor()
...
@caja_bp.route('/eliminar_carrito/<int:idcarrito>')
...
@caja_bp.route('/mostrarcarrito', methods=['POST'])
@proteger_ruta
def mostrarcarrito():
    cliente_id = session.get('cliente_id')
    cur = mydb.cursor()
...
@caja_bp.route('/convertir_venta', methods=['POST'])
@proteger_ruta
def convertir_venta():
    cliente_id = session.get('cliente_id')
