...
def listar_empleado():
    try:
        ...
        with mydb.cursor() as cur:
            ...
            s = """
                SELECT v.idventa, p.nombreproducto, p.precio, v.pago, c.nombrecliente, e.nombreempleado, v.horainicial
                FROM ventas v
                INNER JOIN empleados e ON v.idempleado = e.idempleado
                INNER JOIN clientes c ON v.idcliente = c.idcliente
                INNER JOIN productos p ON v.idproducto = p.idproducto
                WHERE e.idempleado = %s AND e.idempleado = %s  # Verificación adicional
                ORDER BY v.horainicial DESC
            """
            cur.execute(s, (idempleado, idempleado))
        ...
        # No necesitas cerrar explícitamente el cursor, ya que el bloque "with" lo hace automáticamente
