...
def agregar_empleado():
    if request.method == 'POST':
        ...
        cursor = conn.cursor()
        cursor.execute("INSERT INTO empleado (nombre, cargo, correo, usuario, clave) VALUES (%s, %s, %s, %s, %s)", (nombre,  cargo, correo, usuario, clave))
        conn.commit()
        cursor.close()
...
def update_editempleado(id):
    if request.method== 'POST':
        ...
        clave=request.form['clave']
        cur=conn.cursor(cursor_factory=psycopg2.extras.DictCursor)
        cur.execute(""" UPDATE empleado SET nombre=%s, cargo=%s, correo=%s, usuario=%s, clave=%s  WHERE idempleado=%s""", (nombre, cargo, correo, usuario, clave, id))
        conn.commit()
        return redirect(url_for('empleado'))

# Eliminar un empleado
@app.route('/eliminar_empleado/<string:id>')
def delet_editempleado(id):
    cur=conn.cursor(cursor_factory=psycopg2.extras.DictCursor)
    cur.execute('DELETE FROM empleado WHERE idempleado={0}'.format(id))
    conn.commit()
    flash('el empleado se a eliminado satisfactoriamente')
    return redirect(url_for('empleado'))
